...
pub fn check_browser_permission(browser_name: String) -> BrowserPermissionResult {
    #[cfg(target_os = "macos")]
    {
        // Step aside so the macOS automation-permission dialog appears in front of Flint.
...
/// Relinquishes the app's active-application status so the upcoming macOS system
/// permission dialog (TCC) can appear in front of the Flint window.
/// Uses a raw `msg_send!` because `NSRunningApplication::deactivate` exists in
/// the macOS runtime but isn't generated by objc2-app-kit 0.3.x bindings.
#[cfg(target_os = "macos")]
fn deactivate_for_permission() {
    unsafe {
        use objc2::msg_send;
        use objc2_app_kit::NSRunningApplication;
        let app = NSRunningApplication::currentApplication();
        let _: bool = msg_send![&*app, deactivate];
    }
}
